...
int g_timer_init(TIMER_HANDLER callback){
	struct sigaction        sa;
	struct sigevent         se;
	struct GTIMER_STRUCT	*ts;
	struct itimerspec       its;
	...
	memset(&timerStruct, 0, sizeof(struct GTIMER_STRUCT) * GTIMER_MAX );
	memset(&its, 0, sizeof(struct itimerspec));

	sa.sa_flags = SA_SIGINFO;
	sa.sa_flags |= SA_RESTART;
	sa.sa_sigaction = (callback != NULL)? callback: g_timer_handler;
	...
	for(intptr_t i=0; i<GTIMER_MAX; i++){
		ts=&timerStruct[ i ];
		se.sigev_value.sival_ptr = (void*)i;//ts;
